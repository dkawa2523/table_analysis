...
from .platform_adapter_common import PlatformAdapterError, existing_user_properties, resolve_clearml_task
...
def create_pipeline_controller(cfg: Any, *, name: str | None=None, tags: Iterable[str] | None=None, properties: Mapping[str, Any] | None=None, default_queue: str | None=None) -> Any:
    pipeline_utils = load_clearml_pipeline_utils(clearml_enabled=True)
    ...
    task = resolve_clearml_task(controller)
    apply_clearml_task_type(task, clearml_task_type_controller())
    apply_clearml_system_tags(task, ['pipeline'])
    if tag_list:
        apply_clearml_tags(task, tag_list)
    project_name = controller_project or _cfg_value(cfg, 'run.clearml.project_name') or getattr(task, 'project', None)
    ensure_clearml_project_system_tags(project_name, ['pipeline'], remove_tags=['hidden'])
    if properties:
        ...
        setter(task, merged)
    apply_clearml_task_script_override(controller, cfg)
    apply_clearml_pipeline_args(controller, cfg)
    apply_clearml_task_requirements(resolve_clearml_task(controller), resolve_clearml_pipeline_requirements(cfg))
    return controller
