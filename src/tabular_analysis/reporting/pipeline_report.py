...
def _collect_report_payload(pipeline_run: Mapping[str, Any], *, cfg: Any | None, max_models: int) -> dict[str, Any]:
    ...
    requested_jobs = pipeline_run.get('requested_jobs')
    planned_jobs = pipeline_run.get('planned_jobs')
    executed_jobs = pipeline_run.get('executed_jobs')
    disabled_jobs = pipeline_run.get('disabled_jobs')
    ...
    selection_payload = pipeline_run.get('selection', {}) if isinstance(pipeline_run, Mapping) else {}
    disabled_selection = list(pipeline_run.get('disabled_selection') or [])
    ...
    recommended_infer_key = resolve_infer_selection_key(preferred_reference)
    recommended_infer_value = resolve_infer_selection_value(preferred_reference)
    recommended_infer_assignment = resolve_infer_selection_assignment(preferred_reference)
    recommended_ref_kind = resolve_infer_selection_kind(preferred_reference)
    ...
    if disabled_selection:
        actions.append('Review the Selection section if you intentionally restricted preprocess/model/ensemble subsets.')
...
def _render_report_markdown(payload: Mapping[str, Any], *, max_models: int) -> str:
    ...
    lines: list[str] = ['# Pipeline Summary', '', '## Conclusion', f"- grid_run_id: {_format_value(payload.get('grid_run_id'))}", f"- infer_key: {_format_value(summary.get('recommended_infer_key'))}", f"- infer_value: {_format_value(summary.get('recommended_infer_value'))}"]
    if summary.get('recommended_infer_assignment'):
        lines.append(f"- use_in_infer: {_format_value(summary.get('recommended_infer_assignment'))}")
    lines.extend([f"- ref_kind: {_format_value(summary.get('recommended_ref_kind'))}", f"- recommended_registry_model_id: {_format_value(summary.get('recommended_registry_model_id'))}", f"- infer_model_id: {_format_value(summary.get('infer_model_id'))}", f"- infer_train_task_id: {_format_value(summary.get('infer_train_task_id'))}", f"- train_task_ref: {_format_value(summary.get('train_task_ref'))}", f"- recommended_model_id: {_format_value(summary.get('recommended_model_id'))}", f"- primary_metric: {_format_value(summary.get('primary_metric'))}", f"- best_score: {_format_value(summary.get('best_score'))}", f"- status: {_format_value(payload.get('status'))}"])
    ...
    if summary.get('disabled_jobs') is not None:
        lines.append(f"- disabled_jobs: {_format_value(summary.get('disabled_jobs'))}")
    ...
    lines.extend([
        '',
        '## Selection',
        f"- preprocess.requested: {_format_value(selection.get('requested_preprocess_variants'))}",
        f"- preprocess.active: {_format_value(selection.get('active_preprocess_variants'))}",
        f"- preprocess.disabled: {_format_value(selection.get('disabled_preprocess_variants'))}",
        f"- model.requested: {_format_value(selection.get('requested_model_variants'))}",
        f"- model.active: {_format_value(selection.get('active_model_variants'))}",
        f"- model.disabled: {_format_value(selection.get('disabled_model_variants'))}",
        f"- ensemble.requested: {_format_value(selection.get('requested_ensemble_methods'))}",
        f"- ensemble.active: {_format_value(selection.get('active_ensemble_methods'))}",
        f"- ensemble.disabled: {_format_value(selection.get('disabled_ensemble_methods'))}",
    ])
    ...
    lines.extend(['', '## Recommendation', f"- infer_key: {_format_value(recommendation.get('infer_key'))}", f"- infer_value: {_format_value(recommendation.get('infer_value'))}"])
    if recommendation.get('infer_assignment'):
        lines.append(f"- use_in_infer: {_format_value(recommendation.get('infer_assignment'))}")
    lines.extend([f"- ref_kind: {_format_value(recommendation.get('ref_kind'))}", f"- registry_model_id: {_format_value(recommendation.get('registry_model_id'))}", f"- infer_model_id: {_format_value(recommendation.get('infer_model_id'))}", f"- infer_train_task_id: {_format_value(recommendation.get('infer_train_task_id'))}", f"- model_id: {_format_value(recommendation.get('model_id'))}", f"- primary_metric: {_format_value(recommendation.get('primary_metric'))}", f"- best_score: {_format_value(recommendation.get('best_score'))}"])
