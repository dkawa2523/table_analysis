...
def _bootstrap_metric_ci(y_true: Any, y_pred: Any, y_proba: Any | None, *, metric_name: str, task_type: str, n_classes: int | None, n_boot: int, alpha: float, seed: int, beta: float | None, point_estimate: float | None) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    try:
        import numpy as np
    except ImportError as exc:
...
def _apply_resampling(strategy: str, X_train: Any, y_train: Any, *, seed: int) -> tuple[Any, Any, dict[str, Any] | None, str | None]:
    try:
...
def _apply_weight_strategy(*, model_variant: dict[str, Any], task_type: str, y_train: Any, n_classes: int | None, imbalance_cfg: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    class_path = class_path if isinstance(class_path, str) else None
    if not class_path:
...
def _select_best_threshold(y_true: Any, y_proba: Any, *, metric_name: str, grid: list[float], task_type: str, n_classes: int | None, beta: float | None=None) -> tuple[float, float, str]:
...
def _calibrate_classifier(model: Any, X_val: Any, y_val: Any, *, method: str, mode: str) -> Any:
    ...
    return calibrator
def _prepare_calibration_samples(y_true: Any, y_proba: Any, *, n_classes: int | None) -> tuple[Any, Any, str]:
    try:
        import numpy as np
    except ImportError as exc:
...
def _build_calibration_report(y_true: Any, y_proba: Any, *, n_classes: int | None, n_bins: int=10) -> tuple[dict[str, Any], list[float], list[float]]:
    try:
        import numpy as np
    except ImportError as exc:
        raise RuntimeError('numpy is required for calibration reports.') from exc
    (confidence, correct, mode) = _prepare_calibration_samples(y_true, y_proba, n_classes=n_classes)
    ...
    if n_samples == 0:
        report = {'n_samples': 0, 'n_bins': int(n_bins), 'mode': mode, 'ece': None, 'curve': []}
        return (report, [], [])
    ...
    for idx in range(n_bins):
        if counts[idx] <= 0:
            continue
        curve.append({'bin': int(idx), 'count': int(counts[idx]), 'mean_confidence': float(mean_conf[idx]), 'mean_accuracy': float(mean_acc[idx])})
        curve_conf.append(float(mean_conf[idx]))
        curve_acc.append(float(mean_acc[idx]))
    report = {'n_samples': n_samples, 'n_bins': int(n_bins), 'mode': mode, 'ece': ece, 'curve': curve}
