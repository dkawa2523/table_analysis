...
from .infer_support import build_model_reference_payload, resolve_batch_execution_mode
...
def _resolve_batch_settings(cfg: Any) -> dict[str, Any]:
    ...
    return {'chunk_size': chunk_size, 'output_format': output_format, 'write_mode': write_mode, 'max_rows': max_rows}
def _resolve_batch_children_settings(cfg: Any) -> dict[str, Any]:
    ...
    wait_timeout_sec = _to_positive_float(_cfg_value(cfg, 'infer.batch.wait_timeout_sec', None), default=3600.0)
    poll_interval_sec = _to_positive_float(_cfg_value(cfg, 'infer.batch.poll_interval_sec', None), default=10.0)
    return {'max_children': max_children, 'wait_timeout_sec': wait_timeout_sec, 'poll_interval_sec': poll_interval_sec}
...
def _resolve_optimize_settings(cfg: Any) -> dict[str, Any]:
    ...
    if isinstance(contour_params, Sequence) and (not isinstance(contour_params, (str, bytes))):
        ...
        contour_params = None
    return {'n_trials': n_trials, 'direction': direction, 'sampler_name': sampler_name, 'sampler_seed': sampler_seed, 'search_space': search_space, 'objective_keys': objective_keys, 'top_k': top_k, 'wait_timeout_sec': wait_timeout_sec, 'poll_interval_sec': poll_interval_sec, 'history_log_scale': history_log_scale, 'contour_params': contour_params}
...
def _build_optimize_hparams(optimize_settings: Mapping[str, Any] | None) -> dict[str, Any] | None:
    ...
    return {'infer.optimize.n_trials': optimize_settings.get('n_trials'), 'infer.optimize.direction': optimize_settings.get('direction'), 'infer.optimize.sampler': optimize_settings.get('sampler_name'), 'infer.optimize.objective': objective_label, 'infer.optimize.search_space': search_space_summary, 'infer.optimize.top_k': optimize_settings.get('top_k')}
...
def _load_batch_inputs(payload: Any | None, path: str | None, *, max_rows: int | None=None) -> tuple[Any | None, str | None]:
    ...
    if max_rows is not None and max_rows > 0:
        df = df.head(max_rows)
    return (df, str(data_path))
def _resolve_preprocess_columns(preprocess_bundle: Mapping[str, Any]) -> tuple[list[str], list[str], list[str], str | None]:
    ...
    target_column = _normalize_str(columns.get('target_column')) or _normalize_str(preprocess_bundle.get('target_column'))
