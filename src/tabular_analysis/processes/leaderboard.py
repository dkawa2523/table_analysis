...
from ..common.model_reference import (
...
def _write_leaderboard_csv(path: Path, rows: Iterable[dict[str, Any]], *, metric_names: Iterable[str]=()) -> None:
    fieldnames = ['rank', 'model_family', 'ensemble_method', 'n_base_models', 'composite_score', 'best_score', 'primary_metric_ci_low', 'primary_metric_ci_mid', 'primary_metric_ci_high', 'primary_metric', 'primary_metric_source', 'task_type', *[name for name in metric_names], 'infer_selector', 'infer_target', 'reference_kind', 'model_id', 'infer_model_id', 'infer_train_task_id', 'train_task_id', 'preprocess_variant', 'model_variant', 'train_task_ref', 'processed_dataset_id', 'split_hash']
...
def _resolve_infer_selector(entry: Mapping[str, Any]) -> str | None:
    return resolve_infer_selection_key(entry)
def _resolve_infer_target(entry: Mapping[str, Any]) -> str | None:
    return resolve_infer_selection_value(entry)
def _resolve_recommended_infer_payload(entry: Mapping[str, Any]) -> dict[str, str | None]:
    ref_kind = resolve_infer_selection_kind(entry) or _normalize_str(entry.get('reference_kind'))
    return {
        'recommended_ref_kind': ref_kind,
        'recommended_infer_key': resolve_infer_selection_key(entry),
        'recommended_infer_value': resolve_infer_selection_value(entry),
        'recommended_infer_assignment': resolve_infer_selection_assignment(entry),
    }
def _build_recommended_model_payload(*, rank: int, entry: Mapping[str, Any]) -> dict[str, Any]:
    recommended_infer = _resolve_recommended_infer_payload(entry)
    return {
        'rank': rank,
        ...
        'infer_model_id': entry.get('infer_model_id'),
        'infer_train_task_id': entry.get('infer_train_task_id'),
        'reference_kind': entry.get('reference_kind'),
        'ref_kind': recommended_infer.get('recommended_ref_kind'),
        'infer_key': recommended_infer.get('recommended_infer_key'),
        'infer_value': recommended_infer.get('recommended_infer_value'),
...
def _handle_dry_run_without_refs(ctx: Any, cfg: Any, settings: LeaderboardRuntimeSettings, ref_values: dict[str, Any]) -> bool:
    ...
    if settings.clearml_enabled:
        ...
        update_task_properties(ctx, {'recommended_train_task_id': None, 'recommended_model_id': None, 'recommended_registry_model_id': None, 'infer_model_id': None, 'infer_train_task_id': None, 'recommended_ref_kind': None, 'recommended_infer_key': None, 'recommended_infer_value': None, 'excluded_count': 0, 'selection_policy': selection_policy})
    ...
    outputs = {'leaderboard_csv': str(leaderboard_path), 'recommended_train_task_id': None, 'recommended_model_id': None, 'recommended_registry_model_id': None, 'infer_model_id': None, 'infer_train_task_id': None, 'recommended_ref_kind': None, 'recommended_infer_key': None, 'recommended_infer_value': None, 'recommended_composite_score': None, 'excluded_count': 0}
...
def _write_decision_summaries(cfg: Any, ctx: Any, *, clearml_enabled: bool, rows: list[dict[str, Any]], recommended: dict[str, Any], recommended_metrics: dict[str, Any], direction: str, ranking_score_key: str, ranking_direction: str, require_comparable: bool, ref_values: dict[str, Any], excluded: list[dict[str, Any]], warning_messages: list[str], metric_source_priority: list[str], allow_cross_metric_source: bool, allow_ensemble: bool, tie_breaker: str, scoring_normalization: str, scoring_metrics: list[str], scoring_weights: dict[str, float], leaderboard_path: Path) -> tuple[Path, Path]:
    ...
    recommended_infer = _resolve_recommended_infer_payload(recommended)
    decision_lines = ['# Decision Summary', '', '## Infer Run Input', f"- infer_key: {recommended_infer.get('recommended_infer_key')}"]
    decision_lines.append(f"- infer_value: {recommended_infer.get('recommended_infer_value')}")
    if recommended_infer.get('recommended_infer_assignment'):
        decision_lines.append(f"- use_in_infer: {recommended_infer.get('recommended_infer_assignment')}")
    decision_lines.extend([f"- ref_kind: {recommended_infer.get('recommended_ref_kind')}", f"- recommended_registry_model_id: {recommended.get('registry_model_id')}", f"- recommended_train_task_id: {recommended.get('train_task_id')}", f"- recommended_model_id: {recommended.get('model_id')}", '', '## Recommendation Context', f"- infer_model_id: {recommended.get('infer_model_id')}", f"- infer_train_task_id: {recommended.get('infer_train_task_id')}", f"- reference_kind: {recommended.get('reference_kind')}", f"- train_task_ref: {recommended.get('train_task_ref')}", f"- primary_metric: {recommended.get('primary_metric')} ({direction})", f"- primary_metric_source: {recommended.get('primary_metric_source')}", f"- best_score: {_format_float(recommended.get('best_score'))}", f'- ranking_score_key: {ranking_score_key} ({ranking_direction})'])
    if recommended_ci:
...
def _write_leaderboard_out_and_manifest(cfg: Any, ctx: Any, *, clearml_enabled: bool, use_composite: bool, scoring_normalization: str, ref_values: dict[str, Any], recommended: dict[str, Any], excluded: list[dict[str, Any]], non_comparable: list[dict[str, Any]], warning_messages: list[str], rows: list[dict[str, Any]], refs: list[str], require_comparable: bool, top_k: int, scoring_metrics: list[str], scoring_weights: dict[str, float], ranking_score_key: str, ranking_direction: str, direction: str, leaderboard_path: Path, recommendation_path: Path, summary_path: Path, skipped_path: Path, decision_summary_path: Path, decision_summary_json_path: Path) -> None:
    if clearml_enabled:
        ...
        recommended_infer = _resolve_recommended_infer_payload(recommended)
        properties_payload = {'recommended_train_task_id': recommended.get('train_task_id') or None, 'recommended_model_id': recommended.get('model_id'), 'recommended_registry_model_id': recommended.get('registry_model_id'), 'infer_model_id': recommended.get('infer_model_id'), 'infer_train_task_id': recommended.get('infer_train_task_id'), 'recommended_ref_kind': recommended_infer.get('recommended_ref_kind'), 'recommended_infer_key': recommended_infer.get('recommended_infer_key'), 'recommended_infer_value': recommended_infer.get('recommended_infer_value'), 'recommended_primary_metric_source': recommended.get('primary_metric_source'), 'excluded_count': len(excluded), 'selection_policy': selection_policy}
        if recommended.get('composite_score') is not None:
