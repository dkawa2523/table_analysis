...
def init_task_context(cfg, *, stage: str, task_name: str, tags: Optional[list[str]]=None, properties: Optional[dict]=None, task_type: str | None=None, system_tags: Optional[Iterable[str]]=None) -> TaskContext:
    ...
    project_name = _cfg_value(cfg, 'run.clearml.project_name') or _cfg_value(cfg, 'task.project_name') or f'{project_root}/TabularAnalysis/{usecase_id}/{stage}'
    ...
    if not is_clearml_enabled(cfg):
        ...
        return ctx
    try:
        ensure_clearml_names(cfg, project_name=str(project_name), task_name=str(task_name_value), clearml_enabled=True)
        platform_clearml = load_clearml_module(clearml_enabled=True)
        ...
        merged_props = build_clearml_properties(cfg, stage=stage, task_name=str(task_name_value), extra=properties, clearml_enabled=True)
        process = str(merged_props.get('process') or task_name_value or stage)
        merged_tags = build_clearml_tags(cfg, process=process, schema_version=str(merged_props.get('schema_version') or 'unknown'), grid_run_id=merged_props.get('grid_run_id'), retrain_run_id=merged_props.get('retrain_run_id'), extra_tags=_cfg_value(cfg, 'run.clearml.extra_tags') or [], tags=tags)
        ...
        apply_clearml_task_type(task, task_type)
        apply_clearml_task_script_override(task, cfg)
        apply_clearml_system_tags(task, system_tags)
        ...
        existing = existing_user_properties(task)
    ...
    except (PlatformAdapterError, ImportError, AttributeError, RuntimeError, TypeError, ValueError) as exc:
        raise PlatformAdapterError(f'Failed to init ClearML task via ml-platform: {exc}') from exc
    _capture_env_snapshot(ctx)
    return ctx
def save_config_resolved(ctx: TaskContext, cfg) -> Path:
    ...
    if ctx.task is not None:
        try:
            ...
            raise PlatformAdapterError('ml_platform.config.export_config_artifact not available for ClearML runs.') from exc
        try:
...
def update_task_properties(ctx: TaskContext, props: Mapping[str, Any]) -> None:
    ...
    if setter is None:
        raise PlatformAdapterError('ml_platform.integrations.clearml.set_user_properties not found.')
    existing = existing_user_properties(ctx.task)
...
def connect_hyperparameters(ctx: TaskContext, hparams: Mapping[str, Any], *, name: str | None=None) -> None:
    ...
    payload = canonicalize_clearml_section_payload(hparams)
    try:
        if name:
    ...
    if callable(setter):
        ...
        if callable(getter):
            try:
                ...
                existing = []
        merged = dedupe_tags([*existing, *tag_list])
        try:
            setter(merged)
            return
        except RECOVERABLE_CLEARML_ERRORS as exc:
