...
> NOTE:
> - `svr` は回帰専用、`svc` は分類専用
> - `regression_all` は `svr` を含まない。標準 full set は `catboost, elasticnet, extra_trees, gaussian_process, gradient_boosting, knn, lasso, lgbm, linear_regression, mlp, random_forest, ridge, xgboost`
