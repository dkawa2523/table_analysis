...
- [ ] `LOCAL/TabularAnalysis/Pipelines` に visible pipeline template がある
- [ ] `pipeline`
- [ ] `train_model_full`
- [ ] `train_ensemble_full`
- [ ] type が `controller`
- [ ] task tags に `process:pipeline`, `task_kind:template`, `pipeline_profile:<name>` がある
...
- [ ] run controller が `LOCAL/TabularAnalysis/Pipelines` に見える
- [ ] type が `controller`
- [ ] `task_kind:run` がある
...
- [ ] Pipelines タブに template が見える
- [ ] template を clone できる
- [ ] run controller が visible project に残る
- [ ] 推奨モデルと report が取れる
